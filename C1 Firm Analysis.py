...
def create_simple_forecast():
    ...
    try:
        ...
        # Remove any NaN values and ensure all arrays match
        valid_indices = ~np.isnan(turnover)
        turnover = turnover[valid_indices]
        cogs = cogs[valid_indices]
        ebitda = ebitda[valid_indices]
        profit = profit[valid_indices]
        years = [years[i] for i, valid in enumerate(valid_indices) if valid]
        ...
        # Check for unrealistic values (likely data entry errors)
        print(f"Data range check:")
        print(f"  Min turnover: ${min(turnover):,.0f}")
        print(f"  Max turnover: ${max(turnover):,.0f}")
        
        # If we have extreme outliers, flag them
        median_turnover = np.median(turnover)
        for i, val in enumerate(turnover):
            if val > median_turnover * 100 or val < median_turnover / 100:
                print(f"  ⚠️ Potential data issue in {years[i]}: ${val:,.0f}")
    ...
    def method_1_average_growth():
        ...
        if len(turnover) < 2:
            return [turnover[-1]] * 3, "Insufficient data"
            
        # Calculate year-over-year growth rates (filter out extreme values)
        growth_rates = []
        for i in range(1, len(turnover)):
            if turnover[i-1] != 0:  # Avoid division by zero
                growth = (turnover[i] - turnover[i-1]) / turnover[i-1]
                # Filter out extreme growth rates (likely data errors)
                if -0.8 < growth < 3.0:  # Between -80% and +300%
                    growth_rates.append(growth)
        
        if not growth_rates:
            # Fallback to simple average if no valid growth rates
            avg_growth = 0.05  # Default 5% growth
            print("No valid growth rates found, using default 5%")
        else:
            avg_growth = np.mean(growth_rates)
            # Cap the average growth to reasonable bounds
            avg_growth = max(-0.3, min(0.5, avg_growth))  # Between -30% and +50%
    ...
    def method_2_recent_trend():
        ...
        recent_years = min(5, len(turnover))  # Use available data if less than 5 years
        recent_data = turnover[-recent_years:]
        
        if recent_years < 2:
            return [turnover[-1]] * 3, "Insufficient recent data"
        
        # Simple linear trend
        x = np.arange(recent_years)
        y = recent_data
        
        # Calculate slope (trend) - simplified formula
        n = len(x)
        sum_x = np.sum(x)
        sum_y = np.sum(y)
        sum_xy = np.sum(x * y)
        ...
        for i in range(3):
            ...
            forecasts.append(max(0, forecast))  # Ensure positive values
        
        return forecasts, f"Recent Trend (${slope:,.0f}/year)"
    
    def method_3_conservative():
        """Method 3: Conservative estimate"""
        recent_years = min(3, len(turnover))
        last_avg = np.mean(turnover[-recent_years:])
        print(f"Last {recent_years} years average: ${last_avg:,.0f}")
        ...
        for year in range(3):
            forecast = last_avg * (1 + modest_growth) ** (year + 1)
    ...
    try:
        ...
        recent_data = turnover[-recent_years_count:]
        
        ax2.plot(recent_years_plot, recent_data, 'bo-', linewidth=3, markersize=6, label='Recent Historical')
        ...
        if len(turnover) > 1:
            ...
            for i in range(1, len(turnover)):
                if turnover[i-1] != 0:
                    ...
                    growth = max(-100, min(500, growth))  # Between -100% and 500%
                    growth_rates.append(growth)
                else:
                    growth_rates.append(0)
            
            # Only plot if we have reasonable data
            if growth_rates and max(growth_rates) < 1000:
                ax3.bar(growth_years[:len(growth_rates)], growth_rates, alpha=0.7, color='lightblue')
                ax3.axhline(y=0, color='red', linestyle='-', alpha=0.7)
                if growth_rates:
                    avg_growth = np.mean(growth_rates)
                    ax3.axhline(y=avg_growth, color='green', linestyle='--', alpha=0.7, 
        ...
        print("Forecasts calculated successfully, but charts couldn't be displayed")
    ...
    print(f"\nBusiness Insights:")
    print(f"  Current ({years[-1]}): ${current_turnover:,.0f}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if results:
            ...
            print("❌ Forecasting failed - please check your data file")
            
    except Exception as e:
        print(f"❌ Error: {e}")
        print("Please check that your Excel file is accessible and has the correct format")