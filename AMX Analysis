...
# ----- .1. Turnover 
print(pl_usd.iloc[4:5, :14]) # Helper
# Values - FIXED: Extract as array, not DataFrame
t_turnover = pl_usd.iloc[4, 1:14].values.astype(float)  # Changed from iloc[4:5, 1:14]
...
# ----- .2. COGS
print(pl_usd.iloc[6:7, :14]) # Helper 
# Values - FIXED: Extract as array
t_cogs = pl_usd.iloc[6, 1:14].values.astype(float)  # Changed from iloc[6:7, 1:14]
...
# ----- .3. Industrial Margin 
print(pl_usd.iloc[8:9, :14]) # Helper
# Values - FIXED: Extract as array
t_im = pl_usd.iloc[8, 1:14].values.astype(float)  # Changed from iloc[8:9, 1:14]
...
# ----- .4. EBITDA 
print(pl_usd.iloc[18:19, :14]) # Helper 
# Values - FIXED: Extract as array
t_ebitda = pl_usd.iloc[18, 1:14].values.astype(float)  # Changed from iloc[18:19, 1:14]
...
# ----- .5. NET PROFIT 
print(pl_usd.iloc[34:35, :14]) # Helper 
# Values - FIXED: Extract as array
t_nprof = pl_usd.iloc[34, 1:14].values.astype(float)  # Changed from iloc[34:35, 1:14]
...
print((export.head),(export.shape))
print(export.iloc[167:168, :44]) # Helper 
# Values - FIXED: Extract as array and match the length
t_prod_sold = export.iloc[167, 2:15].values.astype(float)  # Adjusted to match 13 columns
...
# Values - FIXED: Extract as array and match the length
t_invent = export.iloc[171, 2:15].values.astype(float)  # Adjusted to match 13 columns
...
# ----- STEP 6: AMX FINANCIAL ANALYSIS SUMMARY
print("\n" + "="*50)
...
# -----  STEP 7: COMPREHENSIVE FINANCIAL VISUALIZATIONS (UPDATED)
...
ax1.set_title('AMX Financial Metrics Overview', fontsize=14, fontweight='bold')
ax1.set_ylabel('Amount (USD)', fontsize=11)
ax1.tick_params(axis='x', rotation=45, labelsize=9)

# Top Right: Key Ratios
ratios = ['Liquidity', 'Interest Coverage', 'ROA (%)']
ratio_values = [liquid_ratio*100, int_cov_ratio, roa/100]
...
ax3.set_title("AMX Turnover Trend", fontsize=14, fontweight='bold')
...
# ----- STEP 8: COMPREHENSIVE TREND ANALYSIS

# Define years and check data consistency
years = list(range(2007, 2020))  # 13 years to match your data
...
ax3.plot(years, t_invent, 'pink', marker='>', linewidth=2.5, label='Inventory')
...
# ----- STEP 9: GROWTH ANALYSIS & SUMMARY INSIGHTS
...
# ----- STEP 10: COMPREHENSIVE SUMMARY STATISTICS CHATGPT
print("\n" + "="*80)
print("COMPREHENSIVE AMX FINANCIAL ANALYSIS SUMMARY")
print("="*80)
...
print("\n" + "="*80)
print("📋 ANALYSIS COMPLETE - Review charts above for visual insights")
print("="*80)